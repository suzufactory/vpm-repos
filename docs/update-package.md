...
1. 配布元のリリースから zip を取得する
   ...
   ```sh
   gh release download <source-tag> --repo <source-repo> \
     --pattern "<zip-name>"
   ```
...
2. zip をこのリポジトリ（vpm-repos）の Releases にアップロードする
   ...
   ```sh
   gh release create <release-tag> --repo suzufactory/vpm-repos \
...
7. そのパッケージの過去バージョンの Release（とタグ）を vpm-repos から削除する

   `vpm.json` は最新バージョンのみを掲載する運用なので、古い Release は残さない。
   現行バージョン以外の `<package-id>` の Release をすべて削除する。

   ```sh
   gh release delete <old-release-tag> --repo suzufactory/vpm-repos \
     --cleanup-tag --yes
   ```

   他パッケージ（例: `scm-*`）の Release は削除しないこと。
